...
// ── Config (ตั้งใน Supabase → Settings → Edge Functions → Secrets) ──────────
...
const PRICE             = '499'
const ACCOUNT_NUMBER    = '1578147760'       // TODO: เปลี่ยนเป็นเลขบัญชีจริง
const ACCOUNT_NAME_TH   = 'บจก. ดับเบิ้ลคราฟ์' // TODO: ชื่อบัญชีผู้รับจริง
const ACCOUNT_TYPE      = '01004'             // ประเภทบัญชี K-BANK

// ── Generate TARN-XXXXXXXX ───────────────────────────────────────────────────
function generateCode(): string {
  const chars = 'ABCDEFGHJKLMNPQRSTUVWXYZ23456789' // ตัด I, O, 0, 1 ออก (อ่านสับสน)
  let suffix = ''
  for (let i = 0; i < 8; i++) {
...
<table width="100%" cellpadding="0" cellspacing="0" border="0"
  ...
  <tr><td align="center" style="padding:48px 48px 36px;border-bottom:1px solid #F0F0F0;">
    <div style="width:64px;height:64px;background:#FFF3EF;border-radius:18px;margin:0 auto 20px;
    ...
    <h1 style="margin:0 0 10px;font-size:26px;font-weight:700;color:#1D1D1F;letter-spacing:-0.5px;line-height:1.2;">
      ขอบคุณที่สั่งซื้อนะคะ
    </h1>
    <p style="margin:0;font-size:15px;color:#6E6E73;line-height:1.5;">
      สวัสดีคุณ <strong style="color:#1D1D1F;">${name}</strong> ·
      PowerPoint Template by Coach Tarn พร้อมให้คุณแล้วค่ะ
    </p>
  </td></tr>
  <!-- Code -->
  <tr><td style="padding:36px 48px;border-bottom:1px solid #F0F0F0;">
    <p style="margin:0 0 14px;font-size:13px;font-weight:600;color:#6E6E73;letter-spacing:0.3px;">Redeem Code ของคุณ</p>
    <table width="100%" cellpadding="0" cellspacing="0" border="0">
      <tr><td style="background:#F5F5F7;border-radius:12px;padding:20px 24px;">
        <span style="font-family:'SF Mono','Fira Code','Courier New',monospace;
                     font-size:26px;font-weight:700;color:#1D1D1F;letter-spacing:4px;display:block;">
          ${code}
        </span>
  ...
  <tr><td style="padding:32px 48px;border-bottom:1px solid #F0F0F0;">
    ...
    <table width="100%" cellpadding="0" cellspacing="0" border="0" style="margin-bottom:14px;">
      <tr>
        ...
        <td style="padding-left:12px;padding-top:3px;font-size:15px;color:#1D1D1F;">
          วาง Code <code style="background:#F5F5F7;border-radius:6px;padding:2px 8px;font-size:13px;color:#D34724;font-weight:700;">${code}</code> ในช่องที่กำหนด
        </td>
      </tr>
    </table>
    <table width="100%" cellpadding="0" cellspacing="0" border="0">
  ...
  <tr><td align="center" style="padding:32px 48px;border-bottom:1px solid #F0F0F0;">
    <a href="${downloadUrl}" target="_blank"
       style="display:inline-block;background:#D34724;color:#ffffff;text-decoration:none;
              font-size:16px;font-weight:600;padding:15px 40px;border-radius:100px;letter-spacing:-0.2px;">
      รับไฟล์สไลด์ →
    </a>
  </td></tr>
  <!-- Order detail -->
...
serve(async (req) => {
  ...
  try {
    ...
    // ── Signed URL สำหรับส่งให้ Slip2Go (อายุ 5 นาที) ──────────────────────
    const { data: signedData } = await supabase.storage
      .from('slips')
      .createSignedUrl(order.slip_url, 300)

    if (!signedData?.signedUrl) {
      return Response.json({ status: 'rejected', reason: 'slip_not_found' }, { status: 400, headers: CORS })
    }
    ...
    const slip = await slip2goRes.json()
    const isPass     = slip?.data?.checkCondition?.isPass
    const transRef   = slip?.data?.transRef
    const reasonCode = slip?.code

    // ── สลิปซ้ำ ─────────────────────────────────────────────────────────────
    if (reasonCode === '200501') {
      // เช็คว่า order นี้เคย verify สำเร็จแล้วหรือยัง (อาจเป็น retry จากลูกค้าเดิม)
    ...
    // ── ไม่ผ่านเงื่อนไข ──────────────────────────────────────────────────────
    if (!isPass) {
      await supabase.from('orders').update({ status: 'rejected', reject_reason: `slip2go_${reasonCode}` }).eq('id', order_id)
      return Response.json({ status: 'rejected', reason: 'slip_invalid', code: reasonCode }, { headers: CORS })
    }

    // ── ผ่านแล้ว — ตรวจ trans_ref ซ้ำในฐานข้อมูลเอง ─────────────────────────
    if (transRef) {
      ...
      if (dupOrder && dupOrder.id !== order_id) {
        ...
        return Response.json({ status: 'rejected', reason: 'duplicate_slip' }, { headers: CORS })
      }
    }

    // ── สร้าง Redeem Code (retry safe) ──────────────────────────────────────
    let redeemCode = ''
    let attempts   = 0
    while (attempts < 5) {
      const candidate = generateCode()
      const { error: insertErr } = await supabase.from('redeem_codes').insert({
        order_id     : order_id,
        customer_name: order.name,
        code         : candidate,
      })
      if (!insertErr) { redeemCode = candidate; break }
      attempts++
    }
